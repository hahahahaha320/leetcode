...
public class Q18_4Sum {
	...
	public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        for(int a=0;a<nums.length-3;a++)	{
        	if(numList.get(a)>target && numList.get(a) > 0)	{
        		break;
        	}
	        for(int i=a+1;i<nums.length-2;i++)	{
	        	Integer need = target-numList.get(a)-numList.get(i);
	        	int j=i+1;
	        	int k=numList.size()-1;
	        	while(j<k)	{
	        		int num = numList.get(j)+numList.get(k);
	        		if(num == need)	{
	        			String key = numList.get(a)+"," +numList.get(i)+"," +numList.get(j)+","+numList.get(k);
	        			if(!set.contains(key))	{
	        				List<Integer> list = new ArrayList<Integer>();
	        				list.add(numList.get(a));
